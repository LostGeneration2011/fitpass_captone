...
export default function StudentContactScreen({ navigation }: any) {
  ...
  const handleSupportChat = async () => {
    try {
      const thread = await chatAPI.createSupportThread();
      navigation.navigate('ChatThread', { threadId: thread.id, title: 'Hỗ trợ Admin', threadType: thread.type });
    } catch (error: any) {
      Toast.show({
        type: 'error',
        text1: 'Lỗi',
        text2: error?.message || 'Không thể tạo chat hỗ trợ',
      });
    }
  };
    ...
    <SafeAreaView className={`flex-1 ${screenClass}`}>
      ...
      >
        <View className="flex-row items-center justify-between mb-6">
          ...
          <TouchableOpacity onPress={handleSupportChat} className="bg-blue-600 px-4 py-2 rounded-lg">
            <Text className="text-white font-semibold">Hỗ trợ Admin</Text>
          </TouchableOpacity>
        </View>
        ...
        {threads.length === 0 ? (
          ...
          threads.map((thread) => {
            ...
            const subtitle = thread.type === 'SUPPORT'
              ? 'Admin Support'
              : isGroup
                ? 'Nhắn tin nhóm lớp'
                : `Giáo viên: ${thread.teacher?.fullName || 'Chưa cập nhật'}`;
